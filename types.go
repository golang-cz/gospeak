...
func (p *parser) parseNamedType(typeName string, typ types.Type) (varType *schema.VarType, err error) {
	...
	case *types.Named:
		...
		if pkgPath == "command-line-arguments" || pkgPath == p.schemaPkgName {
			// "command-line-arguments" package path is autogenerated by Go tool chain. Ignore.
			pkgPath = ""
		}

		// Add this package to importedPaths, if not found.
		if _, ok := p.importedPaths[pkgPath]; !ok {
			p.importedPaths[pkgPath] = struct{}{}
		...
		typeName = pkgName + strings.ToUpper(typeName[0:1]) + typeName[1:]
